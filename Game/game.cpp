...
void Game::Init()
{
    ...
    route_3D_bezier_1D = RouteBezier();
    route_3D_bezier_1D.createRoute3DBezier1D(this, segNum, res, mode);
    route_3D_bezier_1D.cubeStepSize = cube_movement_speed;
}
...
void Game::mouseScrolling(glm::vec3 delta, int mode)
{
    if (pickedShape == -1) {
        ...
        if (pickedShape == route_3D_bezier_1D.firstPointIndex) {
            glm::vec4 first_point = shapes[pickedShape]->getTranslate()[3];
            glm::vec4 cube = shapes[route_3D_bezier_1D.cubeShapeIndex]->getTranslate()[3];
            ...
            if (cube_distance < 2.f) {
                ...
                shapes[route_3D_bezier_1D.cubeShapeIndex]->MyTranslate(glm::vec3(cube_to_point.x, cube_to_point.y, cube_to_point.z), 0);

                // Reset cube position on the 1D Bezier line
                route_3D_bezier_1D.cubeSegment = 0;
                route_3D_bezier_1D.cubeT = 0;

                shapes[route_3D_bezier_1D.cubeShapeIndex]->setRotate(mat4(1));
            }
        }
    }
}
...
void Game::MouseProccessing(int button)
{
    if (pickedShape == -1)
    {
        ...
        if (button == 1)
        {
            ...
            if (pickedShape < route_3D_bezier_1D.cubeShapeIndex) {
                ...
                if (route_3D_bezier_1D.onCurvePoint(pickedShape)) {
                    ...
                    if (pickedShape == route_3D_bezier_1D.firstPointIndex) {
                        ...
                        if (cube_distance < 1.f) {
                            ...
                            shapes[route_3D_bezier_1D.cubeShapeIndex]->MyTranslate(glm::vec3(trans_x.x, trans_x.y, trans_x.z), 0);
                            shapes[route_3D_bezier_1D.cubeShapeIndex]->MyTranslate(glm::vec3(trans_y.x, trans_y.y, trans_y.z), 0);
                    ...
                    if (route_3D_bezier_1D.cState == true) {
            ...
            else if (pickedShape == route_3D_bezier_1D.cubeShapeIndex) {
                // Multiyply by camera rotation
                glm::vec4 trans_x = rot_inverse * glm::vec4(z_buffer_result.x, 0, 0, 1);
                glm::vec4 trans_y = rot_inverse * glm::vec4(0, z_buffer_result.y, 0, 1);
            ...
            if (pickedShape < route_3D_bezier_1D.cubeShapeIndex) {
                ...
                if (route_3D_bezier_1D.onCurvePoint(pickedShape)) {
                    ...
                    if (route_3D_bezier_1D.cState == true) {
                        ...
                        if (second_control_index > route_3D_bezier_1D.firstPointIndex && second_control_index < route_3D_bezier_1D.lastPointIndex) {
